...
public class ShoppingCartResource {
	...
	public ResponseEntity addItem (
		...
		String jerseyId = (String) mapper.get("jerseyId");
		String qty = (String) mapper.get("qty");
		
		User user = userService.findByUsername(principal.getName());
		Jersey jersey = jerseyService.findOne(Long.parseLong(jerseyId));
		
		if(Integer.parseInt(qty) > jersey.getInStockNumber()) {
			return new ResponseEntity("Not Enough Stock!", HttpStatus.BAD_REQUEST);
		}
		
		CartItem cartItem = cartItemService.addJerseyToCartItem(jersey, user, Integer.parseInt(qty));
		
		return new ResponseEntity("Jersey Added Successfully!", HttpStatus.OK);
	}
